...
const TournamentMatch = () => {
  ...
  const getMatchList = () => {
    MatchService.getAll(tournamentId).then((response) => {
  ...
  const scrollToDate = () => {
    ...
    const todayRowIndex = matchList.findIndex((item: any) => dayjs(item.date).format('DD/MM/YYYY') === today);
    if (todayRowIndex !== -1) {
      ...
      <div style={{ display: "flex", justifyContent: "center" }}>
        <div className="team-container">
          <table ref={tableRef} style={{ maxWidth: "1300px" }}>
            ...
            <tbody>
              {matchList.map((match: IMatch, index: number) => (
                <tr key={index + 1}>
              ...
              {emptyMessageBanner && (
                <tr>
                  <td colSpan={userData.role === 'admin' ? 13 : 11}>
                    <div id="main">
                      <div className="fof">
